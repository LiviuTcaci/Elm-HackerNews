...
durationBetween t1 t2 =
    if Time.posixToMillis t1 >= Time.posixToMillis t2 then
        ...
        let
            totalMillis = Time.posixToMillis t2 - Time.posixToMillis t1
            totalSeconds = totalMillis // 1000
            days = totalSeconds // (24 * 60 * 60)
            hours = Basics.modBy 24 (totalSeconds // (60 * 60))
            minutes = Basics.modBy 60 (totalSeconds // 60)
    ...
    let
        ...
        daysPart =
            if duration.days > 0 then
                Just (formatUnit duration.days "day" "days")
            else
                Nothing

        hoursPart =
            if duration.hours > 0 then
                Just (formatUnit duration.hours "hour" "hours")
            else
                Nothing

        minutesPart =
            if duration.minutes > 0 then
                Just (formatUnit duration.minutes "minute" "minutes")
            else
                Nothing

        secondsPart =
            if duration.seconds > 0 then
                Just (formatUnit duration.seconds "second" "seconds")
            else
                Nothing
