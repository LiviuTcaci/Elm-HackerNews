    ...
    let
        ( newState, cmd ) =
            case ( model.state, msg ) of
                ...
                -- Handle the result of fetching individual posts
                ...
                ( Model.LoadedPosts state, ConfigChanged change ) ->
                    let
                        ...
                        filteredPosts =
                            Model.PostsConfig.filterPosts updatedConfig state.posts
                    in
                    ( Model.LoadedPosts { state | config = updatedConfig, posts = filteredPosts }
                    , Effect.NoEffect
                    )

                -- Default case for other states or messages
                ( state, _ ) ->
